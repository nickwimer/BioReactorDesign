    ...
    for repeat in range(2):
        ...
        for ci in range(ncirc):
            for i in range(nsplits):
                ...
                roundfac=1.0
                if(ci<ncirc-1):
                    roundfac=(cb_ratio**(ci))
                outfile.write("( "+str(x)+" "+str(y)+" "+str(cb_center[2]-cb_rad*np.cos(azang)*roundfac)+" ) //"+str(counter)+"\n")  
                counter=counter+1
...
def write_edges(outfile):
    ...
    for ci in range(ncirc):
        ...
        for i in range(nsplits):
            ...
            outfile.write("( "+str(midx)+" "+str(midy)+" "+str(cb_center[2]-cb_rad*np.cos(azang)*roundfac)+" )\n")  
