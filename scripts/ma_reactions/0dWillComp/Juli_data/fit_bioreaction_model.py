...
def setmodelconstants(p):
    ...
    modelConstants["bio_max"] = 27  # kg/m^3

    modelConstants["K_o"] = p[2]  # mol/m^3
...
def loss_function(params, init_cond, t_final, exp_data, normalize=False):
    ...
    F_s = mc["Fs_max"] * sol_y[:, 1] / (sol_y[:, 1] + mc["K_s"])
    F_o = mc["Fo_max"] * sol_y[:, 3] / (sol_y[:, 3] + mc["K_o"])
...
def integrate_solution(params, init_cond, t_final, exp_data):
    ...
    # axs[1, 1].plot(exp_data["time"], exp_data["our"], "ro", label="expr")
    # axs[1, 1].plot(time_int, our, "b-", label="model")
    # axs[1, 1].set_ylabel("oxygen uptake rate", fontsize=14)
...
if __name__ == "__main__":
    ...
    # Initial parameters in order: Fs_max, Fo_max, K_o, Y_xs, Y_ms, Y_os
