...
def write_ofoam_preamble(outfile, react):
    ...
    if react.units == "m":
        ...
        outfile.write("// units: cm\n")
        outfile.write("convertToMeters 0.01;\n\n")
    elif react.units == "mm":
        outfile.write("// units: mm\n")
        outfile.write("convertToMeters 0.001;\n\n")
    elif react.units == "ft":
        outfile.write("// units: ft\n")
        outfile.write("convertToMeters 0.3048;\n\n")
    elif react.units == "in":
        outfile.write("// units: in\n")
        outfile.write("convertToMeters 0.0254;\n\n")
    else:
        raise ValueError("Unsupported units: " + react.units)
    outfile.write(
        "// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //\n\n"
    )
...
def get_baffle_point_of(splti, ci, zi, react):
    ...
    if ci == hub_circ and zi in baff_sections:
...
def write_blocks(outfile, react):
    ...
    hub_circ = react.hub_circ
    rot_circ = react.rot_circ
    mrf_circ = react.mrf_circ
    nvolumes = react.nvolumes
    mrf_volumes = set(react.mrf_volumes)
...
def write_patches(outfile, react):
    ...
    if bottom_inlet:
        ...
        for i in range(nsplits):
            outfile.write("\t\t( ")
            outfile.write(str(get_baffle_point_of(i, rot_ci, zi, react)) + " ")
